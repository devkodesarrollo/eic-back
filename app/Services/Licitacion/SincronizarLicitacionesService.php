...
class SincronizarLicitacionesService
{
        ...
        try {
            ...
            if (!empty($data) && is_array($data)) {
                foreach ($data as $licitacionData) {
                    if (!$this->ExisteLicitacion($licitacionData, $licitaciones)) {
    ...
    function ExisteLicitacion($respuestaApi, $licitaciones) {
        foreach ($licitaciones as $licitacion) {
            if($licitacion->id_del_proceso == $respuestaApi["id_del_proceso"]) return true;
        }
        return false;
    }
}
